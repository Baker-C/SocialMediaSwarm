...
import { ForcePostHistory } from './ForcePostHistory';
...
export function ForcePostSection({ apiBase, accounts, onComplete }: ForcePostSectionProps) {
  ...
  // Tick a clock while a run is in progress so live step/total timings update.
  useEffect(() => {
    if (!running) {
      return;
    }
  ...
  const handleForcePost = useCallback(async () => {
    ...
    let finalized = false;
    const finalize = (outcome: 'success' | 'error', options?: { error?: string | null; postedText?: string | null }) => {
      if (finalized) {
        return;
      }
      finalized = true;
