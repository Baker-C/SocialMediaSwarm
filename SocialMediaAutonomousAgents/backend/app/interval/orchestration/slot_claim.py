"""Interval slot idempotency: reserve before LLM work, release on failure."""
...
def _slot_lock_path(account_id: str, slot: str) -> Path:
    ...
    base = Path(tempfile.gettempdir()) / "sma_interval_slots"
...
def try_reserve_interval_slot(ctx: TickContext, account_id: str) -> tuple[SlotReservation | None, str | None]:
    """
    Reserve the current interval slot for ``account_id`` (scheduled mode only).

    Returns ``(reservation, skip_reason)``. On success, ``last_interval_slot`` is persisted
    ...
    if ctx.mode != "scheduled":
        ...
        return SlotReservation(account=fresh, previous_slot=fresh.last_interval_slot, lock_path=Path()), None

    fresh = reload_account(ctx, account_id)
    if fresh is None:
        return None, "account_not_found"
    if fresh.last_interval_slot == ctx.slot:
        return None, "already_posted_this_interval"
    ...
    with _account_lock(account_id):
        if not _acquire_lock_file(lock_path):
            ...
            if again is not None and again.last_interval_slot == ctx.slot:
                return None, "already_posted_this_interval"
            logger.info(
                "interval slot lock held elsewhere account_id=%s slot=%s",
                account_id,
                ctx.slot,
            )
        ...
        previous_slot = fresh.last_interval_slot
        fresh.last_interval_slot = ctx.slot
        ctx.repo.save(fresh)

        verify = reload_account(ctx, account_id)
        if verify is None or verify.last_interval_slot != ctx.slot:
            _release_lock_file(lock_path)
...
def finalize_interval_slot_reservation(ctx: TickContext, account_id: str) -> None:
...
def release_interval_slot_reservation(ctx: TickContext, account_id: str) -> None:
    ...
    with _account_lock(account_id):
        fresh = reload_account(ctx, account_id)
        if fresh is not None and fresh.last_interval_slot == ctx.slot:
            fresh.last_interval_slot = previous_slot
            ctx.repo.save(fresh)
        _release_lock_file(lock_path)
...
def _acquire_lock_file(lock_path: Path) -> bool:
    try:
        ...
        if _is_stale_lock(lock_path):
            logger.warning("removing stale interval slot lock %s", lock_path)
            _release_lock_file(lock_path)
