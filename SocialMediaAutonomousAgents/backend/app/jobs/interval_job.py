...
def run_interval_job() -> dict:
    """Scheduled posting tick: all active accounts on POST_INTERVAL_MINUTES cadence."""
    if not settings.interval_posting_enabled:
        logger.info("interval_job skipped (INTERVAL_POSTING_ENABLED=false)")
        return {"slot": None, "results": [], "skipped": "interval_posting_disabled"}
    if is_post_quiet_hours():
        reason = quiet_hours_skip_reason() or "quiet_hours"
        logger.info(
            "interval_job skipped (%s; no posts %02d:00–%02d:00 %s)",
            reason,
    ...
    try:
        ...
        logger.info(
            "interval_job mode=%s bypass_cooldown=%s slot=%s accounts=%s",
            mode,
            bypass,
            result.get("slot"),
        ...
        logger.exception("interval_job failed (mode=%s)", mode)
        raise
